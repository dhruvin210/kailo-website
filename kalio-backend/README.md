...
values currently hardcoded in `kalio-frontend`. A clean clone against an empty
...
| **npm** | 9+ |
| **PostgreSQL** | 14+. Use the bundled `docker-compose.yml`, or point at an instance you already run. SQLite is not supported. |
...
**Option A — Docker (nothing installed locally):**
...
password from your `.env`. Data lives in the `kailo-pgdata` volume and survives
...
- `FRONTEND_URL` — the primary origin (default `http://localhost:8080`, which is
  the port `kalio-frontend/vite.config.ts` pins the dev server to).
...
`../kalio-frontend/src/assets` — **once, at seed time.** The running server
...
`kalio-frontend/src/lib/products.ts`. The seed downloads and uploads them so the
