...
How `kalio-frontend` consumes this API. The reads and both forms are wired —
...
Add to `kalio-frontend/.env`:

```
VITE_STRAPI_URL=http://localhost:1337
```

Match whatever port the CMS is actually on — `kalio-backend/.env` on this
