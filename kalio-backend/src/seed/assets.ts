 ...
 *
 * Two sources:
 *   • Local originals that ship with the frontend (`kalio-frontend/src/assets`).
 ...
 *     `public/uploads` and kalio-backend is self-contained. Point
 ...
 *
 * `appRoot` must be the kalio-backend project root — NOT `__dirname`, which in
...
export const assetsRoot = (appRoot: string): string =>
  path.resolve(
    process.env.KAILO_ASSETS_DIR ?? path.join(appRoot, '..', 'kalio-frontend', 'src', 'assets')
  );
