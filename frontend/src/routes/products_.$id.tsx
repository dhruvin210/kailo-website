...
export const Route = createFileRoute("/products_/$id")({
  // The route param is a slug, so this is a filtered collection read rather than a
  // findOne — Strapi 5 addresses entries by documentId. "You may also like" is
  // resolved here too, keeping the whole page server-rendered in one pass.
  loader: async ({ params, context }) => {
    const [product, catalogue] = await Promise.all([
      context.queryClient.ensureQueryData(productBySlugQuery(params.id)),
          ...
          <div>
            <div className="overflow-hidden rounded-3xl bg-muted">
              <img
                ...
                srcSet={product.imagesSrcSet?.[activeImg] ?? product.imageSrcSet}
              ...
              <div className="mt-4 flex gap-3">
                {product.images.map((src: string, i: number) => (
                  ...
                  >
                    <img
                      src={src}
                      srcSet={product.imagesSrcSet?.[i]}
                      // Fixed 5rem square, so the smallest candidate is plenty.
                      sizes="5rem"
                      alt=""
            ...
            <div className="mt-3 flex items-center gap-2 text-sm">
              <div className="flex gap-0.5 text-primary">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className={`h-4 w-4 ${i < Math.round(product.rating) ? "fill-primary" : ""}`}
                  />
                ))}
              </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-0.5 text-primary">
                            {Array.from({ length: 5 }).map((_, k) => (
                              <Star
                                key={k}
                                className={`h-3.5 w-3.5 ${k < rv.r ? "fill-primary" : ""}`}
                              />
                            ))}
                          </div>
                        </div>
